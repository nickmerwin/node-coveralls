...
const getBaseOptions = cb => {
  ...
  if (process.env.CODEBUILD_BUILD_ARN) {
    options.service_name = 'AWS CodeBuild';
    options.service_job_id = process.env.CODEBUILD_BUILD_ID;

    if (process.env.CODEBUILD_WEBHOOK_TRIGGER && process.env.CODEBUILD_WEBHOOK_TRIGGER.startsWith('pr/')) {
      options.service_pull_request = process.env.CODEBUILD_WEBHOOK_TRIGGER.substr(3);
    }

    git_commit = process.env.CODEBUILD_RESOLVED_SOURCE_VERSION;
    git_branch = process.env.CODEBUILD_WEBHOOK_HEAD_REF && process.env.CODEBUILD_WEBHOOK_HEAD_REF.startsWith('refs/heads/') ?
      process.env.CODEBUILD_WEBHOOK_HEAD_REF.substr(11) :
  ...
  if (process.env.COVERALLS_GIT_COMMIT) {
    git_commit = process.env.COVERALLS_GIT_COMMIT;
  }

  if (process.env.COVERALLS_GIT_BRANCH) {
  ...
  if (process.env.COVERALLS_PROJECT_ROOT_PATH) {
    options.project_root_path = process.env.COVERALLS_PROJECT_ROOT_PATH;
  }
