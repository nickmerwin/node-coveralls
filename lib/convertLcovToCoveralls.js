...
const convertLcovFileObject = (file, filepath, projectRootPath) => {
  ...
  return {
    name: path.relative(projectRootPath || rootpath, path.resolve(rootpath, file.file)).split(path.sep).join('/'),
    source,
    coverage,
    branches
  };
  ...
  lcovParse(input, (err, parsed) => {
    ...
    parsed.forEach(file => {
      ...
      if (fs.existsSync(currentFilePath)) {
        postJson.source_files.push(convertLcovFileObject(file, filepath, options.project_root_path));
      }
    });
